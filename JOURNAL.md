...
| 2026-04-27 | Migration mémoire hybride : CLAUDE.md réduit 126L → 55L, .claude/rules/ créé (7 règles path-scoped : docker, security-env, destructive-ops, storage-backup, deploy-git, journal-tracking, proxmox), credentials.md → CREDENTIALS.md (convention MAJUSCULES), .brain/SESSION_INBOX.md local supprimé (remplacé par DEV/.brain/SESSION_INBOX.md racine). |
| 2026-04-28 | infra(agents): déploiement 4 subagents Claude Code dans `.claude/agents/` (docker-validator, compose-builder, vpn-checker + Opus `destructive-gate` sur `rm`/`docker volume prune`/`docker system prune`) + hook PreToolUse Bash gate destructif — `isolation: true` |
| 2026-04-29 | infra(hooks): extension regex destructif (rsync --delete, dd if=, mkfs) + PostToolUse docker-compose.yml/.env → arr-config-audit — restructuration globale hooks ~/.claude/hooks/ |
| 2026-04-30 | Correction hooks `.claude/settings.json` — lecture stdin JSON. Les 3 hooks (PreToolUse Bash, PostToolUse Edit\|Write, PostToolUse MultiEdit) lisaient `process.env.TOOL_INPUT_COMMAND` et `process.env.TOOL_INPUT_FILE_PATH` qui n'existent jamais. Réécriture avec pattern stdin JSON (`i.tool_input.command` / `i.tool_input.file_path`). Bonus : regex `.env` corrigée (espace parasite avant `$`). Logique métier préservée : gate destructive, docker-compose, VPN, .env. |
| 2026-05-03 | infra(three-brain): intégration three-brain dans CLAUDE.md — risk-paths déclarés (`docker-compose*.yml`, `.env*`, `**/*.sh`, `Makefile`), section Three-Brain Stack ajoutée au workspace CLAUDE.md, hooks `three-brain-log.sh` + `three-brain-claude-session.sh` documentés dans `.brain/PLAYBOOKS/hooks-strategy.md` |
| 2026-05-03 | infra(three-brain): complétion setup three-brain — table de routing complète + `Skill("three-brain")` ajoutés au CLAUDE.md (parity workspace), `enabledPlugins` codex+gemini dans `.claude/settings.json`, `three-brain-out/log.md` créé pour log par session projet |
| 2026-05-05 | infra(hooks): audit hooks — P1 `hookEventName` ajouté 9 sorties, `isD` étendu (`docker volume rm`), `isVpn` reserré (patterns spécifiques vs générique `/vpn/i`), `2>/dev/null` → `2>>~/.claude/hooks-arrstack.log` ; 4 agents : préfixes `arr-stack/` retirés |
| 2026-05-05 | infra(hooks): P3 — PostToolUse externalisé dans `.claude/hooks/post-edit.mjs` (ESM) ; matchers Edit|Write + MultiEdit fusionnés ; regex `isVpn` resserrée (`openvpn-guide.md` ne matche plus) |
